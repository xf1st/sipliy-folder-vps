...
function cloudPage(username) { // v3 — multiselect + upload progress + disk fix
  ...
  '.is-downloading{opacity:.55}' +
  '.is-downloading .new-badge{animation:pulse-dl 1.8s ease-in-out infinite}' +
  '@keyframes pulse-dl{0%,100%{opacity:1}50%{opacity:.5}}' +
...
'<div id="toast" style="display:none;flex-direction:column;gap:14px;box-sizing:border-box">' +
  ...
  'function showChangelogModal(){var m=document.getElementById("modal-changelog");if(!m)return;document.getElementById("changelog-ver").textContent="' + SITE_VERSION + '";var b=document.getElementById("changelog-body");var h="<ul style=\'margin:0;padding-left:20px;display:flex;flex-direction:column;gap:10px\'>";h+="<li><b>🗂 Файлы больше не исчезают раньше срока:</b> Система теперь запоминает точное время загрузки файла. Срок хранения отсчитывается с момента добавления в систему, а не с даты создания на диске.</li>";h+="<li><b>🎬 Видео с длительностью:</b> Плеер теперь сразу показывает длительность и перемотку — файлы автоматически оптимизируются для браузера (faststart) при первом открытии.</li>";h+="<li><b>🖼 Быстрые превью:</b> Миниатюры теперь сохраняются постоянно (не в /tmp) — после перезагрузки сервера не нужно генерировать заново.</li>";h+="<li><b>⚡ Мгновенные обновления:</b> Прогресс загрузок в реальном времени через SSE.</li>";h+="<li><b>🔔 Push-уведомления:</b> Уведомление о завершении загрузки даже если вкладка закрыта.</li>";h+="</ul>";b.innerHTML=h;m.style.display="flex";localStorage.setItem("last_seen_changelog_version","' + SITE_VERSION + '");}' +
...
'function toggleDashUrlBatchMode(){' +
  ...
  'function monTile(label,value,sub,color){return \'<div style="flex:1 1 140px;min-width:140px;background:rgba(255,255,255,.04);border-radius:12px;padding:12px 14px"><div style="font-size:11px;color:#958ea0;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">\'+label+\'</div><div style="font-size:22px;font-weight:800;color:\'+(color||"#e4e1e6")+\'">\'+value+\'</div><div style="font-size:11px;color:#958ea0;margin-top:2px">\'+(sub||"")+\'</div></div>\';}' +
  'function pctColor(p){if(p>=90)return "#ff6b6b";if(p>=70)return "#f59e0b";return "#10b981";}' +
  'function fmtUptime(s){s=Math.floor(s);var d=Math.floor(s/86400),h=Math.floor((s%86400)/3600),m=Math.floor((s%3600)/60);if(d>0)return d+"\\u0434 "+h+"\\u0447";if(h>0)return h+"\\u0447 "+m+"\\u043c";return m+"\\u043c";}' +
  'function loadAdminMonitor(){clearTimeout(window._adminMonTimer);fetch("/api/admin/monitor").then(function(r){if(!r.ok)throw new Error("no");return r.json();}).then(function(d){var sec=document.getElementById("admin-monitor-section");if(!sec)return;sec.style.display="block";renderAdminMonitor(d);if(currentPath==="__settings__"){window._adminMonTimer=setTimeout(loadAdminMonitor,3000);}}).catch(function(){var sec=document.getElementById("admin-monitor-section");if(sec)sec.style.display="none";});}' +
  'function renderAdminMonitor(d){var sys=d.system||{},disk=d.disk||{},aria=d.aria||{};var h="";h+=\'<div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:16px">\';h+=monTile("\\u0417\\u0430\\u0433\\u0440\\u0443\\u0437\\u043a\\u0430 CPU",(sys.loadAvg?sys.loadAvg[0].toFixed(2):"\\u2014"),(sys.cpuCount||0)+" \\u044f\\u0434\\u0435\\u0440",pctColor(sys.loadAvg&&sys.cpuCount?sys.loadAvg[0]/sys.cpuCount*100:0));h+=monTile("\\u041e\\u0417\\u0423",(sys.memPercent||0)+"%",fmtSize(sys.memUsed||0)+" / "+fmtSize(sys.memTotal||0),pctColor(sys.memPercent||0));if(disk.percent!=null)h+=monTile("\\u0414\\u0438\\u0441\\u043a",disk.percent+"%",fmtSize(disk.used||0)+" / "+fmtSize(disk.total||0),pctColor(disk.percent));h+=monTile("\\u0421\\u043a\\u043e\\u0440\\u043e\\u0441\\u0442\\u044c aria2",fmtSpeed(aria.downloadSpeed||0),(aria.numActive||0)+" \\u0430\\u043a\\u0442\\u0438\\u0432\\u043d\\u044b\\u0445",null);h+=monTile("\\u041e\\u043d\\u043b\\u0430\\u0439\\u043d",(d.sse?d.sse.count:0),"SSE \\u0441\\u043e\\u0435\\u0434\\u0438\\u043d\\u0435\\u043d\\u0438\\u0439",null);h+=monTile("\\u0410\\u043f\\u0442\\u0430\\u0439\\u043c",fmtUptime(sys.uptime||0),"Node "+(sys.nodeVersion||""),null);h+=\'</div>\';' +
  '  h+=\'<div class="transfer-section-title" style="margin-bottom:8px">\\u0421\\u0435\\u0439\\u0447\\u0430\\u0441 \\u043a\\u0430\\u0447\\u0430\\u0435\\u0442\\u0441\\u044f (\\u0432\\u0441\\u0435 \\u043f\\u043e\\u043b\\u044c\\u0437\\u043e\\u0432\\u0430\\u0442\\u0435\\u043b\\u0438)</div>\';var dls=d.downloads||[];if(!dls.length){h+=\'<div style="color:#958ea0;font-size:12px;margin-bottom:16px">\\u041d\\u0435\\u0442 \\u0430\\u043a\\u0442\\u0438\\u0432\\u043d\\u044b\\u0445 \\u0437\\u0430\\u0433\\u0440\\u0443\\u0437\\u043e\\u043a</div>\';}else{for(var i=0;i<dls.length;i++){var t=dls[i];h+=\'<div style="background:rgba(255,255,255,.03);border-radius:10px;padding:10px 12px;margin-bottom:8px">\';h+=\'<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span style="background:var(--accent-color);color:#fff;font-size:11px;font-weight:700;padding:2px 8px;border-radius:20px">\'+H(t.user)+\'</span><span style="font-size:13px;color:#e4e1e6;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1" title="\'+H(t.name)+\'">\'+(t.isTorrent?"\\ud83e\\udde0 ":"")+H(t.name)+\'</span><span style="font-size:12px;color:#958ea0;white-space:nowrap">\'+t.progress+\'%</span></div>\';h+=\'<div class="progress-track"><div class="progress-fill" style="width:\'+t.progress+\'%"></div></div>\';h+=\'<div style="display:flex;gap:14px;font-size:11px;color:#958ea0;margin-top:5px"><span>\'+fmtSize(t.downloaded)+" / "+fmtSize(t.size)+\'</span><span>\'+fmtSpeed(t.speed)+\'</span><span>\'+t.connections+" \\u0441\\u043e\\u0435\\u0434."+\'</span>\'+(t.seeders!=null?\'<span>\'+t.seeders+" \\u0441\\u0438\\u0434\\u043e\\u0432</span>":"")+\'</div>\';h+=\'</div>\';}}' +
  '  var med=d.activeMedia||[];if(med.length){h+=\'<div class="transfer-section-title" style="margin:14px 0 8px">\\u041c\\u0435\\u0434\\u0438\\u0430-\\u0437\\u0430\\u0433\\u0440\\u0443\\u0437\\u043a\\u0438 (yt-dlp)</div>\';for(var j=0;j<med.length;j++){var m=med[j];h+=\'<div style="background:rgba(255,255,255,.03);border-radius:10px;padding:10px 12px;margin-bottom:8px"><div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span style="background:#8b5cf6;color:#fff;font-size:11px;font-weight:700;padding:2px 8px;border-radius:20px">\'+H(m.user)+\'</span><span style="font-size:13px;color:#e4e1e6;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">\'+H(m.name)+\'</span><span style="font-size:12px;color:#958ea0">\'+m.progress+\'%</span></div><div class="progress-track"><div class="progress-fill" style="width:\'+m.progress+\'%"></div></div><div style="font-size:11px;color:#958ea0;margin-top:5px">\'+H(m.status)+(m.speed?" \\u00b7 "+H(m.speed):"")+\'</div></div>\';}}' +
  '  h+=\'<div class="transfer-section-title" style="margin:14px 0 8px">\\u0414\\u0438\\u0441\\u043a \\u043f\\u043e \\u043f\\u043e\\u043b\\u044c\\u0437\\u043e\\u0432\\u0430\\u0442\\u0435\\u043b\\u044f\\u043c</div>\';var us=d.users||[];for(var k=0;k<us.length;k++){var u=us[k];var quotaBytes=u.quotaGb!=null?u.quotaGb*1024*1024*1024:(disk.total||0);var upct=quotaBytes>0?Math.min(100,Math.round(u.diskUsed/quotaBytes*100)):0;h+=\'<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px"><span style="color:#e4e1e6;font-weight:700">\'+H(u.username)+(u.isAdmin?\' <span style="color:var(--accent-light);font-weight:400">\\u00b7 admin</span>\':"")+\'</span><span style="color:#958ea0">\'+fmtSize(u.diskUsed)+(u.quotaGb!=null?" / "+u.quotaGb+" \\u0413\\u0411":"")+\'</span></div><div class="progress-track"><div class="progress-fill" style="width:\'+upct+\'%"></div></div></div>\';}' +
  '  var act=d.activity||[];if(act.length){h+=\'<div class="transfer-section-title" style="margin:14px 0 8px">\\u041f\\u043e\\u0441\\u043b\\u0435\\u0434\\u043d\\u0438\\u0435 \\u0441\\u043e\\u0431\\u044b\\u0442\\u0438\\u044f</div>\';h+=\'<div style="max-height:240px;overflow-y:auto">\';for(var a=0;a<act.length;a++){var e=act[a];var tm=e.ts?new Date(e.ts).toLocaleString("ru-RU"):"";h+=\'<div style="display:flex;gap:10px;font-size:12px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.05)"><span style="color:#958ea0;white-space:nowrap;min-width:120px">\'+H(tm)+\'</span><span style="color:var(--accent-light);font-weight:700;min-width:70px">\'+H(e.user)+\'</span><span style="color:#cbc3d7;flex:1">\'+H(e.action)+(e.details?" \\u2014 "+H(e.details):"")+\'</span></div>\';}h+=\'</div>\';}document.getElementById("admin-monitor-body").innerHTML=h;}' +
...
'function openMediaViewer(){' +
  ...
  '      stage.innerHTML=\'<video id="mv-media" crossorigin="anonymous" playsinline controls style="max-height:100%" \' + autoplayAttr + \'><source src="\' + previewSrc + \'" type="video/mp4"></video>\';' +
  '    }else{' +
