...
'<div id="toast" style="display:none;flex-direction:column;gap:14px;box-sizing:border-box">' +
  ...
  'function showChangelogModal(){var m=document.getElementById("modal-changelog");if(!m)return;document.getElementById("changelog-ver").textContent="' + SITE_VERSION + '";var b=document.getElementById("changelog-body");var h="<ul style=\'margin:0;padding-left:20px;display:flex;flex-direction:column;gap:10px\'>";h+="<li><b>🎬 Видео с длительностью:</b> Плеер теперь сразу показывает длительность и перемотку — файлы автоматически оптимизируются для браузера (faststart) при первом открытии.</li>";h+="<li><b>🖼 Быстрые превью:</b> Миниатюры теперь сохраняются постоянно (не в /tmp) — после перезагрузки сервера не нужно генерировать заново.</li>";h+="<li><b>⚡ Мгновенные обновления:</b> Прогресс загрузок в реальном времени через SSE.</li>";h+="<li><b>🔔 Push-уведомления:</b> Уведомление о завершении загрузки даже если вкладка закрыта.</li>";h+="<li><b>📁 Конфликты файлов:</b> При загрузке — выбор: заменить, пропустить или сохранить оба.</li>";h+="</ul>";b.innerHTML=h;m.style.display="flex";localStorage.setItem("last_seen_changelog_version","' + SITE_VERSION + '");}' +
